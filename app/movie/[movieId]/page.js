...
const page = async ({ params }) => {
  ...
  return (
    <div className=" w-full h-auto px-[0.5rem] mb-8">
      <div
        className="movie_container w-full h-auto flex flex-col 
      justify-center items-center text-center gap-12"
      >
        <div
          className="w-[100%] flex flex-col lg:flex-row lg:justify-start lg:gap-6 items-center justify-center"
          style={{
            background: `linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)) ,url(${movieBackdroppath});`,
        ...
        >
          <div className="">
            <img
              src={moviePoster}
              alt="Movie_Poster"
              className="mx-auto lg:mx-0  sm:w-[310px] sm:h-[480px] md:w-[340px] md:h-[500px] lg:w-[540px] lg:h-[520px] xlg:w-[35vw] xlg:h-[38vw]"
            />
          </div>
          ...
          <div className="w-full  text-left flex flex-col gap-6 ">
            ...
            <div className="flex gap-3">
              ...
              {movieGenre.map((genre) => (
                <h1 key={Math.random() * 30}>{genre.name}</h1>
              ))}
            </div>

            <h1>
              <em>MovieTime</em>: {movieTime}
            </h1>
          </div>
        </div>
        ...
        <div className="cast flex flex-col gap-4 scroll-thing  ">
          ...
          <div className="w-[80vw] flex overflow-x-auto gap-8 rounded-tl-lg rounded-tr-lg scroll-thing">
            {movieCast.map((actor) => (
              <div
                key={actor.id}
                className="flex-shrink-0 w-[180px] h-[350px] rounded-lg bg-slate-100 rounded-tl-lg rounded-tr-lg mb-4 text-left"
              >
                <Link href={`/actors/${actor.id}`}>
                  <img
                    src={tmdbimagefetchurl + actor.profile_path}
                    className="w-[100%] object-cover rounded-tl-lg rounded-tr-lg"
                    alt="movie_poster"
                  />
                  <h1 className="text-black text-lg font-semibold">
                    {actor.name ?? actor.originale_name}
                  </h1>
                  <h1 className="text-sm text-zinc-600 font-semibold">
                    {actor.character}
                  </h1>
                </Link>
              </div>
